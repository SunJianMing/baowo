  ...
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITYOBJECTSCORE:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label='评价对象ID'  >
          <template slot-scope="scope">
              {{scope.row.activityObjectId}}
          </template>
      </el-table-column>
      <el-table-column label='评价人员ID'  >
          <template slot-scope="scope">
              {{scope.row.assessorId}}
          </template>
      </el-table-column>
      <el-table-column label='分数，100表示10分，null表示没有打分'  >
          <template slot-scope="scope">
              {{scope.row.score}}
          </template>
      </el-table-column>
      <el-table-column label='是否有备注 0/没有,1/有'  >
          <template slot-scope="scope">
              {{scope.row.isRemark}}
          </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITYOBJECTSCORE:VIEW">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <ActivityObjectScoreView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></ActivityObjectScoreView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityObjectScoreEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></ActivityObjectScoreEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityObjectScoreEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></ActivityObjectScoreEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  import ActivityObjectScoreEdit from './activityObjectScoreEdit'
  import ActivityObjectScoreView from './activityObjectScoreView'

  export default {
    data() {
      return {
        formListUrl: '/modules/activityObjectScore/activityObjectScoreList',
        formDeleteUrl: '/modules/activityObjectScore/activityObjectScoreDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
          },
    ...
    components: {
      ActivityObjectScoreEdit,
      ActivityObjectScoreView,
      WtAuth
    },
    activated() {
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetActivityObjectScoreDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryActivityObjectScoreDetailData', rowData.id)
