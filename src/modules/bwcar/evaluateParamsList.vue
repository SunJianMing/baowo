<template>
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATEPARAMS:ADD">
          <el-button type="primary" @click="toAdd">新增</el-button>
        </wt-auth>
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATEPARAMS:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATEPARAMS:VIEW">
            <el-button size="mini" @click="toView(scope.row)">查看</el-button>
          </wt-auth>
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATEPARAMS:EDIT">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <EvaluateParamsView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></EvaluateParamsView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <EvaluateParamsEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></EvaluateParamsEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <EvaluateParamsEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></EvaluateParamsEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  import EvaluateParamsEdit from './evaluateParamsEdit'
  import EvaluateParamsView from './evaluateParamsView'

  export default {
    data() {
      return {
        formListUrl: '/modules/evaluateParams/evaluateParamsList',
        formDeleteUrl: '/modules/evaluateParams/evaluateParamsDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
            name: '',
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetEvaluateParamsDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryEvaluateParamsDetailData', rowData.id)
