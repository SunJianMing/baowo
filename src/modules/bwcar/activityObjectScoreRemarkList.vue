  ...
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITYOBJECTSCOREREMARK:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label='评价打分信息ID'  >
          <template slot-scope="scope">
              {{scope.row.activityObjectScoreId}}
          </template>
      </el-table-column>
      ...
      <el-table-column label='视频、语音的时长'  >
          <template slot-scope="scope">
              {{scope.row.ext1}}
          </template>
      </el-table-column>
      <el-table-column label='视频的封面图片'  >
          <template slot-scope="scope">
              {{scope.row.ext2}}
          </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITYOBJECTSCOREREMARK:VIEW">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <ActivityObjectScoreRemarkView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></ActivityObjectScoreRemarkView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityObjectScoreRemarkEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></ActivityObjectScoreRemarkEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityObjectScoreRemarkEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></ActivityObjectScoreRemarkEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  import ActivityObjectScoreRemarkEdit from './activityObjectScoreRemarkEdit'
  import ActivityObjectScoreRemarkView from './activityObjectScoreRemarkView'

  export default {
    data() {
      return {
        formListUrl: '/modules/activityObjectScoreRemark/activityObjectScoreRemarkList',
        formDeleteUrl: '/modules/activityObjectScoreRemark/activityObjectScoreRemarkDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
          },
    ...
    components: {
      ActivityObjectScoreRemarkEdit,
      ActivityObjectScoreRemarkView,
      WtAuth
    },
    activated() {
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetActivityObjectScoreRemarkDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryActivityObjectScoreRemarkDetailData', rowData.id)
