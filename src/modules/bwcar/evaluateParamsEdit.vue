<template>
    <div class="dialog-container wt-dialog-container" v-loading="isEvaluateParamsDetailLoading">
...
<script>
    ...
    export default {
      ...
      data() {
        return {
          form: {
            formUpdateUrl: '/modules/evaluateParams/evaluateParamsUpdate',
            formAddUrl: '/modules/evaluateParams/evaluateParamsAdd'
          },
      ...
      methods: {
          ...
          this.$refs['form'].validate((valid) => {
            if (valid) {
              this.$confirm('是否添加?', '提示', {
                ...
                that.requestAddEvaluateParamsDetail()
              }).catch(() => {
        ...
        onSave() {
          ...
          this.$confirm('是否保存?', '提示', {
            ...
            that.requestUpdateEvaluateParamsDetail()
          }).catch(() => {
            that.isSubmitLoading = false
          })
        },
        /*  ------------------------- 网络请求  -------------------------  */
        requestUpdateEvaluateParamsDetail() {
          const that = this
        ...
        requestAddEvaluateParamsDetail() {
          const that = this
          that.isSubmitLoading = true
          request({
      ...
      computed: {
        ...mapState({
          isEvaluateParamsDetailLoading: state => state.evaluateParams.isEvaluateParamsDetailLoading,
          detailResult: state => state.evaluateParams.evaluateParamsDetailData
        })
      }
    }
</script>
