  ...
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATESURVEYPROPERTY:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label='活动类型id'  >
          <template slot-scope="scope">
              {{scope.row.evaluateTypeId}}
          </template>
      </el-table-column>
      <el-table-column label='评价问卷id'  >
          <template slot-scope="scope">
              {{scope.row.evaluateSurveyId}}
          </template>
      </el-table-column>
      <el-table-column label='活动类型id'  >
          <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATESURVEYPROPERTY:VIEW">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <EvaluateSurveyPropertyView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></EvaluateSurveyPropertyView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <EvaluateSurveyPropertyEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></EvaluateSurveyPropertyEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <EvaluateSurveyPropertyEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></EvaluateSurveyPropertyEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  import EvaluateSurveyPropertyEdit from './evaluateSurveyPropertyEdit'
  import EvaluateSurveyPropertyView from './evaluateSurveyPropertyView'

  export default {
    data() {
      return {
        formListUrl: '/modules/evaluateSurveyProperty/evaluateSurveyPropertyList',
        formDeleteUrl: '/modules/evaluateSurveyProperty/evaluateSurveyPropertyDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
            updateTimeRange: []
    ...
    components: {
      EvaluateSurveyPropertyEdit,
      EvaluateSurveyPropertyView,
      WtAuth
    },
    activated() {
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetEvaluateSurveyPropertyDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryEvaluateSurveyPropertyDetailData', rowData.id)
