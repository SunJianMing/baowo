<template>
  <div class="app-container">
    <el-form size="small" :inline="true" label-width="120px">
        <el-form-item label="用户名，作为pad端登录账号唯一：">
        <el-input v-model="form.formData.userName" placeholder="请输入用户名，作为pad端登录账号唯一"></el-input>
    </el-form-item>
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ASSESSOR:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label='用户名，作为pad端登录账号唯一'  >
          <template slot-scope="scope">
              {{scope.row.userName}}
          </template>
      </el-table-column>
      ...
      <el-table-column label='身高cm'  >
          <template slot-scope="scope">
              {{scope.row.height}}
          </template>
      </el-table-column>
      <el-table-column label='体重kg'  >
          <template slot-scope="scope">
              {{scope.row.weight}}
          </template>
      </el-table-column>
      <el-table-column label='驾龄年'  >
          <template slot-scope="scope">
              {{scope.row.drivingYears}}
          </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ASSESSOR:VIEW">
...
<script>
  ...
  import AssessorEdit from './assessorEdit'
  ...
  export default {
    data() {
      return {
        ...
        formDeleteUrl: '/modules/assessor/assessorDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
            userName: '',
