  ...
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITY:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label='活动状态 0/默认,1/待发布,2/进行中,3/已结束(正常结束),4/已结束(主动结束)'  >
          <template slot-scope="scope">
              {{scope.row.state}}
          </template>
      </el-table-column>
      <el-table-column label='是否显示属性表'  >
          <template slot-scope="scope">
              {{scope.row.isShowProperty}}
          </template>
      </el-table-column>
      <el-table-column label='是否必须完整完成 0/不必完成,1/必须完成'  >
          <template slot-scope="scope">
              {{scope.row.isNeedComplete}}
          </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITY:VIEW">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <ActivityView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></ActivityView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></ActivityEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></ActivityEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  export default {
    data() {
      return {
        formListUrl: '/modules/activity/activityList',
        formDeleteUrl: '/modules/activity/activityDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
            name: '',
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetActivityDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryActivityDetailData', rowData.id)
