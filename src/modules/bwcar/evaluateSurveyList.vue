  ...
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATESURVEY:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_EVALUATESURVEY:VIEW">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <EvaluateSurveyView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></EvaluateSurveyView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <EvaluateSurveyEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></EvaluateSurveyEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <EvaluateSurveyEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></EvaluateSurveyEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  import EvaluateSurveyEdit from './evaluateSurveyEdit'
  import EvaluateSurveyView from './evaluateSurveyView'

  export default {
    data() {
      return {
        formListUrl: '/modules/evaluateSurvey/evaluateSurveyList',
        formDeleteUrl: '/modules/evaluateSurvey/evaluateSurveyDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
            name: '',
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetEvaluateSurveyDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryEvaluateSurveyDetailData', rowData.id)
