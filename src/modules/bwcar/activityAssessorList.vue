  ...
  <div class="app-container">
    ...
    <el-form size="small" class="wt-form-btns" :inline="true" label-width="120px">
      <el-form-item class="search-form-btns">
        ...
        <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITYASSESSOR:DELETE">
    ...
    <el-table :data="result.list" class="wt-table" header-row-class-name="wt-table-header-row"
      ...
      <el-table-column label="操作" align="center" width="250px">
        <template slot-scope="scope">
          <wt-auth class="search-form-btn" hasAnyPremission="MODULES_ACTIVITYASSESSOR:VIEW">
    ...
    <div class="search-form-pagination">
      ...
      <div class="wt-dialog">
          <el-dialog title="查看" :visible.sync="isViewDialogVisible" :close-on-press-escape="false"
              ...
              <ActivityAssessorView @wt-dialog-close="isViewDialogVisible = false" @wt-dialog-refresh="(isViewDialogVisible = false) "
                           ref="viewView" viewType="view"></ActivityAssessorView>
          </el-dialog>
      </div>
      <!--添加-->
      <div class="wt-dialog">
          <el-dialog title="添加" :visible.sync="isAddDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityAssessorEdit @wt-dialog-close="isAddDialogVisible = false"
                           @wt-dialog-refresh="(isAddDialogVisible = false) | requestFormList()"
                           ref="addView" viewType="add"></ActivityAssessorEdit>
          </el-dialog>
      </div>
      <!--编辑-->
      <div class="wt-dialog">
          <el-dialog title="编辑" :visible.sync="isEditDialogVisible" :close-on-press-escape="false"
                     :close-on-click-modal="false" :modal-append-to-body="false" lock-scroll>
              <ActivityAssessorEdit @wt-dialog-close="isEditDialogVisible = false"
                           @wt-dialog-refresh="(isEditDialogVisible = false) | requestFormList()"
                           ref="editView" viewType="edit"></ActivityAssessorEdit>
          </el-dialog>
      </div>
...
<script>
  ...
  import ActivityAssessorEdit from './activityAssessorEdit'
  import ActivityAssessorView from './activityAssessorView'

  export default {
    data() {
      return {
        formListUrl: '/modules/activityAssessor/activityAssessorList',
        formDeleteUrl: '/modules/activityAssessor/activityAssessorDelete',
        form: { //  表单数据
          isListLoading: true,
          formData: {
          },
    ...
    components: {
      ActivityAssessorEdit,
      ActivityAssessorView,
      WtAuth
    },
    activated() {
    ...
    methods: {
      ...
      toAdd() {
        ...
        this.$store.dispatch('resetActivityAssessorDetailData')
      ...
      toView(rowData) {
        ...
        this.$store.dispatch('queryActivityAssessorDetailData', rowData.id)
