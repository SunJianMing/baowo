...
const auth = {
  ...
  actions: {
          ...
          if (moduleResultList !== null && moduleResultList.length > 0) {
            moduleResultList.forEach((item) => {
              ...
              if (moduleParentId === 1) {
                ...
                moduleResultMap[moduleParentId].push({
                  path: moduleUrl,
                  name: moduleUrl,
                  meta: { title: moduleName, icon: '' },
                  component: allRoutes[item.moduleCode]
                })
              }
            })
    ...
    queryRoleModuleListMap({ commit }) {
      return new Promise((resolve, reject) => {
        request({
          url: '/modules/auth/roleModuleList',
          method: 'post',
          data: {}
        }).then(response => {
          //  模块列表组
    ...
    queryPermissionDetailData({ commit }, id) {
      ...
      return new Promise((resolve, reject) => {
        request({
          url: '/modules/auth/permissionDetail',
          method: 'post',
          data: { id: id }
        }).then(response => {
